 ...
 *
 *  Description:
 *      Returns an ACE action to place a unit's map on the vehicle.
 *      The map will be attached to the vehicle with the given offset and orientation.
 *      This function has a local effect.
 *
 *  Parameter(s):
 *      0: ARRAY - Offset of map to the vehicle.
 *      1: ARRAY - Dir and up vectors of the map.
 *      2: CODE - Additional condition when to show action
 *          Passed arguments:
 *          0: OBJECT - Vehicle.
 *          1: OBJECT - Player.
 *          2: ARRAY - Custom args (see param 4)).
 *      3: ARRAY - Custom arguments for the condition.
 *
 *  Returns:
 *      ARRAY - ACE action
 *
 *  Example:
 *      _action = [[-0.205078,1.49292,-0.3325], [[-1,0,0],[0,0.1,1]]] call mts_map_fnc_getPlaceMapOnVehicleAction;
